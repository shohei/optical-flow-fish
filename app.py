...
file = st.sidebar.file_uploader("Upload a movie file")
...
video_tmp_name = tempfile.NamedTemporaryFile().name+'video.mp4'
figure_tmp_name = tempfile.NamedTemporaryFile().name+'fig1.png'

if file:
    open(video_tmp_name, "wb").write(file.getbuffer())
    cap = cv2.VideoCapture(video_tmp_name)
